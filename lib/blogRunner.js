...
function buildSystemPrompt({ format, primaryKeyword, cta }) {
...
${cta ? `- Include CTA: "${cta}"` : ''}`;
  ...
  if (format === 'html') {
    return `You are an AI optimized for SEO writing. Generate comprehensive, well-researched content.
...
export async function advanceBlogJob(sql, job) {
  ...
  const { topic, keywords, cta, format, model, sections, primaryKeyword } = input;
  ...
  try {
    ...
    if (completedSections < sections.length) {
      ...
      const systemPrompt = buildSystemPrompt({ format, primaryKeyword, cta });
    ...
    if (!output.titles || output.titles.length === 0) {
      ...
      try {
        const titleRaw = await callNvidiaChat(
          apiKey,
          model,
          'Return ONLY a JSON array of exactly 3 title strings.',
          titlePrompt,
